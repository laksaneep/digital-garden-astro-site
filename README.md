...
Notes come from the obsidian vault, which is its own git repo. They are
...
`npm run sync` reads from the obsidian vault by default.
...
_not_ enough; the store lives under `node_modules/`.
...
1. **The publish gate.** Only notes with `published: true` are rendered. Default is false.
2. **Type sits on the grid.** `--cell` is 24px; body line-height is exactly 24px, headings
